...
public class RecordAccessPolicy {
    public boolean canView(VisibilityActor actor, Record record) {
        return RecordVisibilityScope.forActor(actor).allows(record.getCreatedBy(), record.getAssignedTo(),
                record.getLastDeputyId(), record.getStatus(), record.getDeletedAt());
    }
