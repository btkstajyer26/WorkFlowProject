...
public record RecordVisibilityScope(UUID actorId, Set<Relation> relations, Set<RecordStatus> statuses) {
    ...
    public static RecordVisibilityScope forActor(VisibilityActor actor) {
        Objects.requireNonNull(actor, "actor");
        if (!actor.permissionCodes().contains("RECORD_VIEW") || actor.hasSystemRole(SystemRoleKey.ADMIN)) {
            return new RecordVisibilityScope(actor.id(), Set.of(), Set.of());
        }
        ...
        return new RecordVisibilityScope(actor.id(), relations, statuses);
    }

    public boolean allows(UUID createdBy, UUID assignedTo, UUID lastDeputyId,
                          RecordStatus status, LocalDateTime deletedAt) {
        if (deletedAt != null) return false;
        return statuses.contains(Objects.requireNonNull(status, "status"))
