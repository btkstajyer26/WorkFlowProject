...
export function ErrorStatePage({ type }: { type: '403' | '404' }) {
  ...
  return (
    <section className="flex min-h-[calc(100vh-13rem)] items-center justify-center py-8">
      <div className="w-full max-w-xl rounded-3xl border border-slate-200 bg-white p-7 text-center shadow-sm sm:p-10">
        <span className={`mx-auto flex size-16 items-center justify-center rounded-2xl ${forbidden ? 'bg-amber-50 text-amber-700' : 'bg-brand-50 text-brand-700'}`}>
          <Icon className="size-8" aria-hidden="true" />
        </span>
        <p className="mt-6 text-sm font-black tracking-[0.2em] text-brand-700">{type}</p>
        ...
        <div className="mt-7 grid gap-3 sm:grid-cols-2">
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-200 px-4 text-sm font-bold text-slate-700 transition hover:bg-slate-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-500"
          >
