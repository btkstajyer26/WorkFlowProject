  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end justify-center bg-slate-950/35 backdrop-blur-[2px] sm:items-center sm:p-4" role="presentation">
      <section
        ...
        className="max-h-[92vh] w-full overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl sm:max-w-lg sm:rounded-2xl sm:p-6"
      >
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h2 id="admin-dialog-title" className="text-lg font-bold text-slate-950">{title}</h2>
            <p id="admin-dialog-description" className="mt-1 text-sm leading-6 text-slate-600">{description}</p>
          </div>
          <button
            ref={closeButtonRef}
            type="button"
